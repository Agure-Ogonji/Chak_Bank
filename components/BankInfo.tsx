...
const BankInfo = ({account, appwriteItemId, type}: BankInfoProps) => {
  ...
  return (
    <div className={cn(`bank-info ${colors.bg}`,{
        "shadow-sm border-blue-700": type === "card" && isActive, "rounded-xl": type === "card", "hover:shadow-sm cursor-pointer": type === "card"})} onClick={handleBankChange}>
        ...
        <div className='flex w-full flex-1 flex-col justify-center gap-1'>
            <div className='bank-info_content'>
                ...
                {type === "full" && (
                    <p className={`text-12 rounded-12 px-3 py-1 font-medium text-blue-700 ${colors.subText} ${colors.lightBg}`}>
                        {account.subtype}
                    </p>
                )}
            </div>
